...
void startBLE() {
  ...
  // Security: bond with "Just Works" — no PIN required on host side
  ...
  // Advertising — explicit flags are critical for pairing
  BLEAdvertisementData advData;
  advData.setFlags(0x06);                    // LE General Discoverable + no BR/EDR
  advData.setAppearance(HID_KEYBOARD);
  advData.setCompleteServices(BLEUUID(HID_SERVICE_UUID_16));

  BLEAdvertisementData scanData;
  scanData.setName("ESP32 MacroPad");        // name in scan response keeps adv packet small
