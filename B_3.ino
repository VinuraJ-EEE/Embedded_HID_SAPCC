...
//
//  ENCODER — interrupt-driven (not polled)
//  ────────────────────────────────────────
//  Polling inside a loop with BLE delays causes missed pulses.
//  Instead, the CLK pin fires an ISR on every CHANGE edge.
//  The ISR atomically increments/decrements a counter; the loop
//  drains that counter and sends the accumulated scroll ticks.
//
//  Button — millis() debounce (no delay() calls)
// ─────────────────────────────────────────────────────────────────────────────

// Encoder interrupt state
volatile int encDelta   = 0;   // accumulated ticks (+ = CW, - = CCW)
int          encLastA   = LOW; // last CLK level seen by ISR
...
// ISR — runs on every CLK edge, reads DT to determine direction
void IRAM_ATTR encoderISR() {
  int a = digitalRead(ENC_CLK);
  int b = digitalRead(ENC_DT);
  if (a != encLastA) {
    // Standard quadrature: if CLK and DT differ → CW, same → CCW
    encDelta += (a != b) ? 1 : -1;
    encLastA  = a;
  }
}
...
void handleEncoderButton() {
  ...
  if ((now - btnDebounceTs) >= BTN_DEBOUNCE_MS) {
    if (btnLastStable == HIGH && raw == LOW) {
      winRun("devmgmt.msc");   // Open Device Manager on press
    }
    btnLastStable = raw;
  }
}
...
void setup() {
  ...
  // Encoder — interrupt-driven (see encoderISR above)
  // Use INPUT_PULLUP instead of INPUT if your board lacks external pull-ups
  pinMode(ENC_CLK, INPUT);
  pinMode(ENC_DT,  INPUT);
  pinMode(ENC_SW,  INPUT_PULLUP);
  encLastA = digitalRead(ENC_CLK);
